// src/components/shared/TodoColumn.tsx
...
interface TodoColumnProps {
  todos: TaskTodo[];
  selectedDate?: Date; // Se c'è, siamo in DayPage. Se non c'è, siamo in HomePage!
...
const TodoColumn: React.FC<TodoColumnProps> = ({ todos, selectedDate, onToggleTodo, onSelectTask, onAddTaskClick }) => {
  ...
  // 1. MANTENIAMO IL TOGGLE PER DIVIDERE LE DUE SCHEDE
  ...
  // 2. CALCOLO NOTIFICHE SEMPLIFICATO E FUNZIONANTE PER ENTRAMBE LE PAGINE
  const pendingWithoutDate = todos.some(t => t.deadline === 'Nessuna' && !t.done);
  const pendingWithDate = todos.some(t => t.deadline !== 'Nessuna' && !t.done);
  const showNotificationDot = showWithDeadline ? pendingWithoutDate : pendingWithDate;

  // 3. FILTRAGGIO E ORDINAMENTO PERFETTO
  const getSortedTodos = () => {
    const priorityWeights = { Alta: 3, Media: 2, Bassa: 1 };
    
    // Il filtro ora fa SOLO una cosa: divide in due schede.
    // Non tocca più le date, perché DayPage e HomePage gli mandano già le date giuste!
    let filteredTodos = todos.filter(task => 
      showWithDeadline ? task.deadline !== 'Nessuna' : task.deadline === 'Nessuna'
    );
    ...
    return filteredTodos.sort((a, b) => {
      ...
      // 2. Ordinamento per priorità se siamo nella DayPage, o se l'utente lo ha cliccato nella Home
      const currentSortMode = selectedDate ? 'priority' : sortMode;
      if (currentSortMode === 'priority') {
        const diff = priorityWeights[b.priority] - priorityWeights[a.priority];
        if (diff !== 0) return diff;
      }
      
      // 3. A parità di priorità, ordina cronologicamente
  ...
  return (
    // FIX SCHIACCIAMENTO: w-full e min-w-0
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-gray-200 w-[90%] p-6 text-center animate-fadeIn transform transition-all scale-100">
              ...
              <div className="flex gap-3">
                ...
                <button 
                  type="button"
                  onClick={(e) => {
                    onToggleTodo(taskToConfirmClosure!, e);
      ...
      <div className="flex flex-col flex-1 min-h-0 w-full min-w-0">
        <div className="flex items-center justify-between border-b pb-2 mb-4 shrink-0 w-full">
          ...
          <div className="flex gap-2 shrink-0">
            ...
            {/* BOTTONE MOSTRA CON/SENZA DATA + PALLINO NOTIFICA INCROCIATO */}
            ...
            {/* BOTTONE ORDINAMENTO: Visibile solo nella HomePage */}
            {!selectedDate && (
              <button 
        ...
        {/* IL CONTENITORE MISURABILE DEI TODO - w-full per non schiacciarsi */}
        <div ref={listContainerRef} className="flex-1 min-h-0 overflow-hidden space-y-3 w-full min-w-0">
          {visibleTodos.length === 0 ? (
            <div className="p-4 text-center text-sm text-gray-500 italic whitespace-normal">
                <p className="mt-1">
                {!selectedDate && !showWithDeadline 
                  ? "Nessuna idea o progetto in sospeso" 
                  : "Non ci sono task in programma"}
              </p>
            </div>
            ) : (
