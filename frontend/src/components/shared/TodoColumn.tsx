...
const TodoColumn: React.FC<TodoColumnProps> = ({ todos, selectedDate, onToggleTodo, onSelectTask, onAddTaskClick }) => {
  ...
  const showNotificationDot = showWithDeadline 
    ? todos.some(t => t.deadline === 'Nessuna' && !t.done) 
    : todos.some(t => t.deadline !== 'Nessuna' && !t.done);

  // Filtriamo solo per data/non data. Il sort vero e proprio lo fa taskUtils.ts!
  const filteredTodos = todos.filter(task => showWithDeadline ? task.deadline !== 'Nessuna' : task.deadline === 'Nessuna');
  const sortedTodos = sortTasks(filteredTodos, selectedDate ? 'priority' : sortMode);
                  ...
                  onClick={(e) => {
                    ...
                    if (!task.done && task.hasActiveSubtasks) {
                      ...
                      confirm({
                        ...
                        isDestructive: false,
                        onConfirm: () => onToggleTodo(task.id, e) // Eseguiamo il toggle se conferma
                      });
                    } else {
