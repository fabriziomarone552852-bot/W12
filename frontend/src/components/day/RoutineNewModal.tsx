...
const RoutineNewModal: React.FC<RoutineNewModalProps> = ({ isOpen, onClose, routineToEdit, onSave }) => {
    ...
    if (isOpen) {
      if (routineToEdit) {
        setForm({
          ...
          immagine_url: routineToEdit.immagine_url || '',
          piu_volte: routineToEdit.target_completamenti > 1,
          target_completamenti: routineToEdit.target_completamenti || 1
        });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload = {
      titolo: form.titolo,
      tipo: 'R', // Forzato a Routine come richiesto
