...
import { useTasks } from '../../context/TasksContext'; // <-- AGGIUNTO: Importiamo il collegamento diretto al DB per le task!
...
interface CalendarColumnProps {
  ...
  // AGGIUNTO: Ora il bottone o il doppio clic possono inviare una data!
...
const CalendarColumn: React.FC<CalendarColumnProps> = ({ todos, events, onSelectEvent, onAddEventClick }) => {
  ...
  // AGGIUNTO: Peschiamo i dati puri delle task dal database
  ...
  const getItemsForMonthDate = (dayNumber: number) => {
    ...
    // MODIFICA: Invece di usare "todos", filtriamo "rawTasks"
    const dayTasks = (rawTasks || [])
      .filter((t: any) => {
        if (t.fatto) return false; // Nascondiamo quelle già fatte
        if (!t.data_scadenza) return false; // Mostriamo solo quelle con scadenza
        // Verifichiamo se la scadenza coincide con questo giorno del calendario
        return t.data_scadenza.substring(0, 10) === dateKey;
      })
      .map((t: any) => ({
        title: t.titolo,
        type: 'task' as const,
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 h-full flex flex-col relative">
            ...
            {Array.from({ length: mainDaysInMonth }).map((_, i) => {
              ...
              return (
                <div 
                  ...
                  // IL DOPPIO CLIC!
                  onDoubleClick={() => onAddEventClick && onAddEventClick(dateKey)}
      ...
      {view === 'Mese' && (
        <div className="absolute bottom-7 right-7 z-40 pointer-events-none">
          <button 
            // Invia una data vuota (undefined) quando premi il bottone grande
            onClick={() => onAddEventClick && onAddEventClick()}
            className="px-6 py-2.5 bg-white border-2 border-dashed border-gray-300 rounded-xl text-gray-500 hover:border-blue-500 hover:text-blue-500 hover:bg-blue-50 active:scale-95 transition-all flex justify-center items-center font-bold text-sm gap-2 pointer-events-auto"
          >
