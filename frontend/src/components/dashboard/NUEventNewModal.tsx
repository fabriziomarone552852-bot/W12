...
// FUNZIONE MAGICA PER IL FUSO ORARIO: Risolve il problema della mezzanotte!
...
interface NewEventModalProps {
  ...
  onEventSaved: () => void; 
  initialDate?: string | Date | null; // <--- AGGIUNTO: Permette di ricevere la data
}

const NewEventModal: React.FC<NewEventModalProps> = ({ isOpen, onClose, dbCategories, onCategoryAdded, eventToEdit, onEventSaved, initialDate }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if (eventToEdit) {
        ...
        // Legge la stringa magica di ricorrenza salvata dal database
        let freq = 'WEEKLY';
        let interval = '1';
        let until = '';
        let isRec = false;
        ...
        // --- INIZIO NUOVA LOGICA PER LA DATA ---
        
        // 1. Prepariamo la data in formato testo come prima cosa
        let dataInizioTesto = getLocalDateString(); 

        if (initialDate) {
          if (typeof initialDate === 'string') {
            dataInizioTesto = initialDate;
            ...
            // Estraiamo anno, mese e giorno a mano per evitare problemi
            const anno = initialDate.getFullYear();
            const mese = String(initialDate.getMonth() + 1).padStart(2, '0');
            const giorno = String(initialDate.getDate()).padStart(2, '0');
            
            dataInizioTesto = `${anno}-${mese}-${giorno}`; 
            setPickerMonthDate(initialDate);
          }
        } else {
          setPickerMonthDate(new Date());
        }

        // 2. Usiamo solo la parola di testo pulita e sicura per il modulo
        setNewEventForm({
          titolo: '', 
          descrizione: '', 
          data_inizio: dataInizioTesto, // Niente più errori qui!
          data_fine: '', 
          ora_inizio: '', 
          ora_fine: '', 
          category: '', 
  ...
  const handleSaveCategory = async () => {
    ...
    // --- NUOVO: Validazione colore a prova di bomba ---
    let coloreValido = newCategoryForm.colore.trim();
    if (!/^#[0-9A-Fa-f]{6}$/.test(coloreValido)) {
      // Se l'utente ha pasticciato con il testo, forziamo un blu di default
      // per salvare la richiesta e non prendere un 400 Bad Request
      coloreValido = '#3B82F6'; 
    }
    ...
    try {
      const res = await fetch(apiUrl('/categories'), {
        method: 'POST',
        // --- NUOVO: Usiamo gli authHeaders standard ---
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ name: nomePulito, colore: coloreValido, genre: 2 })
      });
      ...
      if (res.ok) {
        ...
        console.error("Il server ha rifiutato i dati:", errData);
      }
    } catch (err) {
  ...
  const handleSalvaNuovoEvento = async (e: React.FormEvent) => {
    ...
    // Costruiamo la stringa magica per la ripetizione leggendola dalla frase dell'interfaccia
    let rruleString = null;
    if (isRecurrent) {
      rruleString = `FREQ=${rruleFreq};INTERVAL=${rruleInterval || 1}`;
      if (rruleUntil) {
        const dataPulita = rruleUntil.replace(/-/g, ''); 
        rruleString += `;UNTIL=${dataPulita}T235959`; // Z tolta per evitare problemi di fuso orario
      }
    }
    ...
    const pacchettoPerIlServer = {
      ...
      rrule: rruleString // Spediamo la nostra regola!
    };

    try {
      // Puliamo l'ID dai cloni generati dalla ricorrenza
      const veroId = eventToEdit ? String(eventToEdit.id).split('-')[0] : null;

      if (eventToEdit) {
        // Usa il superpotere della RAM per AGGIORNARE!
        await updateEvent(veroId!, pacchettoPerIlServer);
      } else {
        // Usa il superpotere della RAM per CREARE!
        await addEvent(pacchettoPerIlServer);
      }

      // 1. NON chiamiamo più onEventSaved() perché la RAM si è già aggiornata da sola!
      // 2. Chiudiamo semplicemente la finestra.
      onClose();
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      <div className="flex gap-4 items-start w-full max-w-5xl justify-center pointer-events-none" onClick={(e) => e.stopPropagation()}>
        ...
        {/* LA MODIFICA È QUI: max-w-md è diventato max-w-lg (un po' più largo) */}
