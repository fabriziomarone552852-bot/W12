...
const EventDetailModal: React.FC<EventDetailModalProps> = ({ 
  ...
  // Capiamo se l'evento ha una data/ora di fine da mostrare
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      {isDeleteDialogOpen && (
        ...
        >
          ...
          >
            ...
            {/* NUOVO AVVISO PER EVENTI RICORRENTI */}
            {selectedEvent.rrule && (
      ...
      <div className="flex gap-4 items-start w-full max-w-md justify-center pointer-events-none">
        <div 
          className="bg-white rounded-2xl shadow-2xl w-full overflow-hidden transform transition-all animate-fadeIn pointer-events-auto flex-shrink-0"
          onClick={(e) => e.stopPropagation()}
        >
          {/* INTESTAZIONE: Colore, Categoria e Bottoni */}
          <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
            <div className="flex items-center gap-2">
              {/* ETICHETTA CATEGORIA (Ora è identica a quella delle Task!) */}
              <span 
                className={`px-2 py-1 text-[10px] font-bold rounded-md uppercase ${getTextColorForBackground(selectedEvent.categoryColor)}`}
                style={{ backgroundColor: selectedEvent.categoryColor || '#9CA3AF' }}
              >
            ...
            <div className="flex items-center gap-1">
              <button title="Modifica evento" onClick={onEditClick} className="p-1.5 text-gray-400 hover:text-blue-500 hover:bg-blue-50 rounded-lg transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
              </button>
              <button title="Elimina evento" onClick={() => setIsDeleteDialogOpen(true)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
          ...
          <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-gray-50 [&::-webkit-scrollbar-thumb]:bg-gray-300 [&::-webkit-scrollbar-thumb]:rounded-full">
            <div>
              ...
              {/* BLOCCO DATA / ORA (Rosso, compatto, intelligente) */}
              <div className="flex flex-wrap items-center gap-2 mt-2 text-sm font-bold text-red-500">
                <span>
                  Data: {dataInizio} {selectedEvent.time && ` ${selectedEvent.time}`}
                </span>
                ...
                {haFine && (
                  <>
                    ...
                    <span>
                      {/* Mostra la data di fine SOLO se è diversa da quella di inizio */}
