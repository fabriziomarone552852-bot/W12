...
const WeekGrid: React.FC<WeekGridProps> = ({ state, events, tasks = [], onDayClick, onSelectEvent, variant = 'classic' }) => {
  ...
  // Classe dinamica per le colonne:
  // - classic: 8 colonne uguali (grid-cols-8)
  // - detailed: 1 colonna fissa da 50px per l'ora + 7 colonne uguali per i giorni
          ...
          {daysOfWeekData.map((day, dIdx) => {
            ...
            return (
              ...
              >
                ...
                {rawDayEvents.map(({ ev, seg }, idx) => {
                  ...
                  return (
                    <React.Fragment key={`${ev.id}-${day.dateStr}-${idx}`}>
                      {variant === 'detailed' ? (
                        // VISUALIZZAZIONE DETTAGLIATA: Testo dell'evento
                        <div 
                          onClick={(e) => { e.stopPropagation(); onSelectEvent(ev); }}
                          className="absolute rounded p-1 text-[10px] sm:text-[11px] font-medium leading-tight overflow-hidden shadow-sm border-l-4 cursor-pointer hover:brightness-95 transition-all"
                          style={{ 
                            top: seg.top, 
                            height: seg.height, 
                            left: `calc(2% + ${overlapIdx * 5}px)`, // Piccolo scostamento se ci sono overlap
                            width: `calc(96% - ${overlapIdx * 5}px)`,
                        ...
                        >
                          <div className="truncate font-bold">{ev.title || ev.title}</div>
                        </div>
                      ) : (
                        // VISUALIZZAZIONE CLASSICA (HOMEPAGE): Pallini colorati
                        <>
                          {isMultiDay && (
                            <div 
                ...
                {/* RENDERING DELLE TASK IN SCADENZA (Solo in variant detailed) */}
                {variant === 'detailed' && dayTasks.length > 0 && (
                  <div className="absolute bottom-1 left-1 right-1 flex flex-col gap-1 z-20 max-h-[30%] overflow-y-auto custom-scrollbar">
                    {dayTasks.map(task => (
                      <div 
                        key={task.id} 
                        className={`text-[9px] sm:text-[10px] rounded px-1.5 py-0.5 truncate border-l-[3px] shadow-sm flex items-center gap-1 ${
                          task.fatto 
                      ...
                      >
                        ...
                        <span className="truncate">{task.titolo || task.titolo}</span>
                      </div>
                    ))}
                  </div>
                )}
