...
// 1. PRENDIAMO L'ARCHIVIO COMPLETO E GLI STRUMENTI DALLA BACHECA
  const { tasks, addTask, updateTask } = useTasks();

  // 2. TRADUCIAMO AL VOLO TUTTE LE TASK PER IL MODALE
  const tuttiITodos = Array.isArray(tasks) ? tasks.map((t: any) => ({
    id: t.id,
    title: t.titolo,
    done: t.fatto,
  ...
  const maxDepth = (user as any)?.max_subtask_depth_user || 3;
  ...
  useEffect(() => {
    if (isOpen) {
      if (taskToEdit) {
        ...
        // Se ha un padre, apriamo automaticamente il pannellino laterale
        setIsSubtaskPanelOpen(!!taskToEdit.parent_id);
      } else {
        // SE STIAMO CREANDO: Partiamo da zero
        setNewTaskForm({
      ...
      // QUANDO SI CHIUDE: Chiudiamo tutti i menù a tendina per pulizia
 ...
 const handleSaveCategory = async () => {
    // 1. NUOVO CONTROLLO: Vediamo se il nome esiste già
    // Usiamo .trim() per togliere eventuali spazi vuoti iniziali/finali
    // e .toLowerCase() per non farci ingannare tra "Palestra" e "palestra"
    const nomePulito = newCategoryForm.name.trim();
    
    if (!nomePulito) return; // Se è vuoto, non fa niente

    const esisteGia = dbCategories.some(
      (cat: any) => cat.name.toLowerCase() === nomePulito.toLowerCase()
    );

    if (esisteGia) {
      // Se esiste, accendiamo l'avviso che hai già costruito e blocchiamo tutto!
      setAlertMessage("Questa categoria è già presente nel database. Scegli un nome diverso!");
      return; 
    }

    // 2. SE NON ESISTE: Procediamo col normale salvataggio
    try {
      const res = await fetch(apiUrl('/categories'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ name: nomePulito, colore: newCategoryForm.colore, genre: 1 })
      });
      if (res.ok) {
  ...
  const handleSalvaNuovaTask = async (e: React.FormEvent) => {
    ...
    try {
      if (taskToEdit) {
        // SE STIAMO MODIFICANDO: Usiamo updateTask!
        await updateTask(taskToEdit.id, pacchettoPerIlServer);
      } else {
        // SE STIAMO CREANDO: Usiamo addTask!
      ...
      result.push(
        <div 
          ...
          onClick={() => {
            ...
            setNewTaskForm(prev => ({
              ...prev, 
              parent_id: isSelected ? '' : task.id.toString() 
            }));
          }}
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      {/* --- AVVISO CENTRALE (Sovrascrive tutto) --- */}
      {alertMessage && (
        <div 
          className="fixed inset-0 bg-gray-900/60 backdrop-blur-md flex items-center justify-center z-[60] p-4 pointer-events-auto"
          onClick={(e) => {
            e.stopPropagation(); // <-- LO SCUDO! Impedisce la chiusura del form
            setAlertMessage(null); // Chiude solo questo avviso cliccando fuori dal bianco
          }}
        >
          <div 
            ...
            onClick={(e) => e.stopPropagation()} // <-- ALTRO SCUDO per chi clicca dentro la parte bianca
          >
            ...
            <button 
              type="button"
              onClick={() => setAlertMessage(null)} // Chiude solo l'avviso
      ...
      <div className="flex gap-4 items-start w-full max-w-5xl justify-center pointer-events-none" onClick={(e) => e.stopPropagation()}>
        ...
        {/* --- 1. MODALINO LATERALE SINISTRO (SCELTA TASK PADRE) --- */}
        {isSubtaskPanelOpen && (
          <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden animate-fadeIn pointer-events-auto flex-shrink-0 flex flex-col">
            ...
            <div className="max-h-[60vh] overflow-y-auto overflow-x-hidden [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar]:h-1.5 [&::-webkit-scrollbar-track]:bg-gray-50 [&::-webkit-scrollbar-thumb]:bg-gray-300 [&::-webkit-scrollbar-thumb]:rounded-full">
              ...
              {tuttiITodos.filter(t => !t.done && !t.parent_id).length === 0 && (
                <div className="p-4 text-center text-sm text-gray-500 italic whitespace-normal">
                  Nessuna task principale trovata.
                </div>
              )}
            </div>
          </div>
        ...
        {/* --- 2. LA FINESTRA PRINCIPALE DELLA TASK AL CENTRO --- */}
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-visible transform transition-all animate-fadeIn relative pointer-events-auto flex-shrink-0">
          <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-2xl">
            <h3 className="text-lg font-extrabold text-gray-800 uppercase tracking-wider">
              {taskToEdit ? 'Modifica Task' : 'Nuova Task'}
            </h3>
          ...
          <form onSubmit={handleSalvaNuovaTask} className="p-6 space-y-4">
            ...
            {/* --- CASELLA DA SPUNTARE PER SOTTOTASK --- */}
            <div className="bg-gray-50 p-3 rounded-xl border border-gray-100">
              <div className="flex items-center gap-2">
                <input 
                  ...
                  onChange={(e) => {
                    setIsSubtaskPanelOpen(e.target.checked);
                    if (!e.target.checked) setNewTaskForm({...newTaskForm, parent_id: ''});
                  }}
                  className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500 cursor-pointer"
                />
                <label htmlFor="isSubtaskToggle" className="text-sm font-bold text-gray-700 cursor-pointer select-none">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="relative">
                ...
                <div onClick={() => { setIsCategoryDropdownOpen(!isCategoryDropdownOpen); setIsPriorityDropdownOpen(false); setIsDatePickerOpen(false); }} className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm bg-white cursor-pointer flex justify-between items-center hover:border-blue-500 transition-colors">
                  <div className="flex items-center gap-2">
                    <span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: dbCategories.find(c => c.name === newTaskForm.category)?.colore || '#9CA3AF' }}></span>
        ...
        {/* --- 3. MODALINO LATERALE DESTRO (NUOVA CATEGORIA) --- */}
