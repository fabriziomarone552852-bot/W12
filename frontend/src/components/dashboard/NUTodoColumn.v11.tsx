...
const TodoColumn: React.FC<TodoColumnProps> = ({ todos, onToggleTodo, onSelectTask, onAddTaskClick }) => {
  ...
  // STATO PER L'ORDINAMENTO: 'chrono' (Scadenza) o 'priority' (Priorità)
  const [sortMode, setSortMode] = useState<'chrono' | 'priority'>('chrono');
  // STATO PER FILTRARE SOLO LE TASK CON SCADENZA
  ...
  const getSortedTodos = () => {
    ...
    // 1. FILTRIAMO: Separiamo le task in base all'interruttore
    const filteredTodos = todos.filter(task => {
      if (showWithDeadline) {
        // Se l'interruttore è acceso, mostra solo le task in cui la data non è "Nessuna"
        return task.deadline !== 'Nessuna'; 
      } else {
        // Se l'interruttore è spento, mostra SOLO le task senza data
        return task.deadline === 'Nessuna';
      }
    });

    // 2. ORDINIAMO le task che sono "sopravvissute" al filtro
    return filteredTodos.sort((a, b) => {
      ...
      const dateA = new Date(a.dateStr).getTime() || 0; // || 0 evita errori se la data non c'è
  ...
  // --- LOGICA DI PAGINAZIONE (ora basata sui todos ordinati) ---
  const totalPages = Math.ceil(sortedTodos.length / TASKS_PER_PAGE);
  const startIndex = (currentPage - 1) * TASKS_PER_PAGE;
  const visibleTodos = sortedTodos.slice(startIndex, startIndex + TASKS_PER_PAGE);

  // Helper per cambiare ordinamento e resettare la pagina
  const toggleSortMode = () => {
    setSortMode(prev => prev === 'chrono' ? 'priority' : 'chrono');
    setCurrentPage(1); // Riporta l'utente alla prima pagina dopo aver riordinato
  };

  return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 h-full flex flex-col justify-between relative">
              ...
              <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex items-center justify-center z-50 rounded-xl">
                <div className="bg-white rounded-2xl shadow-xl border border-gray-200 w-[90%] p-6 text-center animate-fadeIn transform transition-all scale-100">
                  ...
                  <div className="flex gap-3">
                    <button 
                      type="button"
                      onClick={() => setTaskToConfirmClosure(null)} // Annulla
                    ...
                    <button 
                      type="button"
                      onClick={(e) => {
                        onToggleTodo(taskToConfirmClosure, e as any); // Conferma la chiusura
                        setTaskToConfirmClosure(null); // Chiudi l'avviso
                      }}
  ...
  {/* HEADER CON TITOLO E PULSANTI */}
        <div className="flex items-center justify-between border-b pb-2 mb-4">
          ...
          <div className="flex gap-2">
            ...
            {/* BOTTONE: Filtro Data (Solo Icone) */}
            <button 
              onClick={() => {
            ...
            >
              {showWithDeadline ? (
                // Icona Calendario Normale
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
              ) : (
                // Icona Calendario con la X al centro
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            ...
            {/* BOTTONE: Ordinamento (Solo Icona Rotante) */}
            <button 
            ...
            >
              {/* Qui avviene la magia della rotazione: rotate-180 */}
              <svg 
        ...
        <div className="space-y-3 min-h-[228px] flex flex-col">
            ...
            <div 
              ...
              className={`flex items-center justify-between group cursor-pointer border h-16 px-3 rounded-xl shadow-sm hover:shadow-md transition-all gap-3 ${
                task.isPromotedSubtask 
                  ? 'bg-red-100/50 border-red-200 hover:border-red-300 hover:bg-red-50/50' // Sfondo AZZURRO CHIARO per le sottotask promosse
                  : 'bg-gray-50 border-gray-200 hover:border-blue-300 hover:bg-white'    // Sfondo GRIGIO per i padri normali
              }`}
            >
              <div className="flex items-center gap-3 flex-1 overflow-hidden">
                <input 
                  ...
                  onChange={() => {}} 
                  onClick={(e) => {
                    e.stopPropagation(); // Ferma il click per non far aprire i dettagli della task
                    if (!task.done && task.hasActiveSubtasks) {
                      // Se sto per completarla MA ha sottotask attive, blocco e mostro l'avviso
                      setTaskToConfirmClosure(task.id);
                    } else {
                      // Altrimenti la completo subito
              ...
              {/* ETICHETTA DATA CON ALLARME SOTTOTASK E COLORI PRIORITÀ */}
              <span className={`text-[10px] font-bold px-2 py-1 rounded-md whitespace-nowrap flex-shrink-0 ml-2 transition-colors flex items-center gap-1 ${
                      task.done 
                        ? 'text-gray-500' // Se è fatta, diventa grigia
                      : task.isUrgentFromSubtask 
                        ? 'bg-red-600 text-white' // Se c'è un allarme, rosso scuro e scritta bianca
                      : task.priority === 'Alta'
                        ? 'bg-red-100 text-red-700' // Priorità Alta: Rosso
                      : task.priority === 'Media'
                        ? 'bg-orange-100 text-orange-700' // Priorità Media: Arancione
                        : 'bg-yellow-100 text-yellow-700' // Priorità Bassa: Giallo
              }`}>
                {task.deadline}
              </span>
            </div>
