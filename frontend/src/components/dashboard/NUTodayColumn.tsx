...
const TodayColumn: React.FC<TodayColumnProps> = ({ events, onSelectEvent }) => {
  ...
  const todayEvents = events.filter(e => {
    // Se manca la data, lo ignoriamo
    if (!e.dateStr) return false; 
    
    const inizio = e.dateStr;
    // Se non ha una data di fine, vuol dire che dura un giorno solo (quindi fine = inizio)
    const fine = e.endDateStr || e.dateStr; 
    
    // Controlliamo se "Oggi" è compreso tra l'inizio e la fine!
