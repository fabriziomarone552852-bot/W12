...
const CalendarColumn: React.FC<CalendarColumnProps> = ({ todos, events }) => {
  ...
  const getFirstDayIndex = (year: number, month: number) => {
    ...
    return index === 0 ? 6 : index - 1; // Lunedì = 0
  };
  ...
  // Variabili per il Calendario Principale (Vista Mese)
  ...
  const getItemsForMonthDate = (dayNumber: number) => {
    ...
    const dayEvents = events.filter(e => e.dateStr === dateKey).map(e => ({ title: e.title, type: 'event', category: e.category }));
    return { items: [...dayTasks, ...dayEvents], dateKey };
  };

  // Variabili per il Calendario Principale (Vista Settimana)
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex justify-center items-center">
            <div 
              onClick={() => {
                if (view === 'Mese') {
                  ...
                  // Sincronizza il mini-calendario con il mese del lunedì della settimana corrente
                  setPickerMonthDate(new Date(mondayOfWeek.getFullYear(), mondayOfWeek.getMonth(), 1));
                }
            ...
            >
              ...
              <svg xmlns="http://www.w3.org/2000/svg" className={`h-3 w-3 transition-all ${isSelectingDate ? 'text-blue-500 rotate-180' : 'text-gray-300 group-hover:text-blue-500'}`} viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" />
              </svg>
            </div>

            {/* POPUP CUSTOM (Mese Grid 4x3 oppure Settimana Mini-Calendario) */}
            {isSelectingDate && (
              <>
                ...
                <div className="absolute top-full mt-2 left-1/2 transform -translate-x-1/2 bg-white rounded-xl shadow-2xl border border-gray-100 p-4 z-40 w-64 animate-fadeIn">
                  ...
                  {view === 'Mese' ? (
                    /* --- PICKER VISTA MESE (Griglia 4x3) --- */
                    <>
                      <div className="flex justify-between items-center mb-4 px-2">
                        <button onClick={() => setPickerYear(y => y - 1)} className="text-gray-400 hover:text-gray-800 transition-colors">
                        ...
                        {nomiMesiCorto.map((mese, idx) => {
                          ...
                          return (
                            <div key={mese} className="flex justify-center items-center">
                              <button
                                onClick={() => {
                                  setCurrentMonthDate(new Date(pickerYear, idx, 1));
                                  setIsSelectingDate(false);
                                }}
                                className={`w-11 h-11 flex justify-center items-center rounded-full text-xs font-bold transition-all ${
                                  isActive ? 'bg-blue-100 text-blue-700 font-bold' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'
                                }`}
                              >
                                {mese}
                              </button>
                    ...
                    /* --- NUOVO PICKER VISTA SETTIMANA (Mini-Calendario del Mese) --- */
                    <>
                      <div className="flex justify-between items-center mb-4 px-2">
                        <button onClick={() => setPickerMonthDate(d => new Date(d.getFullYear(), d.getMonth() - 1, 1))} className="text-gray-400 hover:text-gray-800 transition-colors">
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" /></svg>
                        </button>
                        <span className="font-bold text-gray-800 text-sm">
                          {nomiMesiLungo[pickerMonthDate.getMonth()]} {pickerMonthDate.getFullYear()}
                        </span>
                        <button onClick={() => setPickerMonthDate(d => new Date(d.getFullYear(), d.getMonth() + 1, 1))} className="text-gray-400 hover:text-gray-800 transition-colors">
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" /></svg>
                        </button>
                      ...
                      {/* Griglia giorni del mini-calendario */}
                      <div className="grid grid-cols-7 gap-1">
                        {/* Spazi vuoti */}
                        {Array.from({ length: getFirstDayIndex(pickerMonthDate.getFullYear(), pickerMonthDate.getMonth()) }).map((_, i) => (
                        ...
                        {Array.from({ length: getDaysInMonth(pickerMonthDate.getFullYear(), pickerMonthDate.getMonth()) }).map((_, i) => {
                          ...
                          // Evidenziamo il giorno se appartiene alla settimana attualmente selezionata
                          ...
                          return (
                            <button
                              key={dayNum}
                              onClick={() => {
                                // Cliccando sul giorno, impostiamo la data alla settimana di appartenenza
                                setCurrentWeekDate(getMondayOfCurrentWeek(cellDate));
                                setIsSelectingDate(false);
                              }}
                              className={`w-7 h-7 flex mx-auto items-center justify-center rounded-full text-xs font-medium transition-colors ${
                                isSelectedWeek 
        ...
        <div className="flex-1 flex flex-col overflow-visible relative">
          ...
          <div className="grid grid-cols-7 gap-1 flex-1 pb-1">
            {Array.from({ length: mainFirstDayIndex }).map((_, i) => (
              <div key={`empty-${i}`} className="p-2 border border-transparent bg-gray-50/30 rounded-lg min-h-[45px]"></div>
            ))}
            ...
            {Array.from({ length: mainDaysInMonth }).map((_, i) => {
              ...
              return (
                <div 
                  ...
                  className="relative p-1.5 border border-gray-100 rounded-lg hover:border-blue-400 hover:bg-blue-50/50 transition-all cursor-pointer min-h-[45px] flex flex-col justify-between group"
                >
                  ...
                  <div className="flex gap-1 justify-center pb-0.5 overflow-hidden h-2">
                    {items.slice(0, 4).map((item, idx) => (
                      <div key={idx} className={`w-1.5 h-1.5 rounded-full ${categoryColors[item.category] ?? 'bg-gray-400'}`}></div>
                    ))}
                  </div>
              ...
              {hours.map((hour, hIdx) => (
                <div key={hIdx} className="grid grid-cols-8 gap-px bg-gray-50 text-[11px] min-h-[35px] items-center">
                  ...
                  {daysOfWeekData.map((day, dIdx) => {
                    const currentHourEvents = events.filter(e => e.dateStr === day.dateStr && e.time === hour);
                    return (
                      <div key={dIdx} className="bg-white h-full border-l border-gray-100 p-0.5 relative group cursor-pointer hover:bg-blue-50/30">
                        {currentHourEvents.map((ev) => (
                          <div 
                            key={ev.id}
                            className={`absolute inset-x-0.5 top-0.5 bottom-0.5 border-l-2 border-blue-500 rounded p-1 text-[9px] font-bold bg-blue-50 text-blue-700 line-clamp-1 leading-none shadow-sm`}
