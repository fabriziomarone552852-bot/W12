...
interface CalendarColumnProps {
  ...
  // Aggiungiamo questa riga per comunicare con la HomePage!
...
const CalendarColumn: React.FC<CalendarColumnProps> = ({ todos, events, onSelectEvent, onAddEventClick }) => {
  ...
  const getItemsForMonthDate = (dayNumber: number) => {
    ...
    const dayTasks = todos
      .filter(t => t.dateStr === dateKey && (t as any).deadline !== 'Nessuna')
      .map(t => ({ 
        title: t.title, type: 'task' as const, category: t.category, categoryColor: (t as any).categoryColor, time: undefined, endTime: undefined 
      }));
    const dayEvents = events
      .filter(e => (e.dateStr || e.endDateStr) === dateKey)
      .map(e => ({ 
        title: e.title, type: 'event' as const, category: e.category, categoryColor: e.categoryColor, time: e.time, endTime: e.endTime 
      }));
      ...
      {view === 'Mese' ? (
        <div className={`flex-1 flex flex-col overflow-visible relative transition-none ${hoveredDay ? 'z-50' : 'z-0'}`}>
          ...
          <div className="grid grid-cols-7 gap-1 flex-1 pb-1">
            {/* CELLE NASCOSTE (Start) */}
            {Array.from({ length: mainFirstDayIndex }).map((_, i) => <div key={`empty-start-${i}`} className="p-2 border-transparent min-h-[45px]"></div>)}
                ...
                <div key={dayNum} onMouseEnter={() => hasItems && setHoveredDay(dateKey)} onMouseLeave={() => setHoveredDay(null)} className="relative p-1.5 border border-gray-100 rounded-lg hover:border-blue-400 hover:bg-blue-50/50 transition-all cursor-pointer min-h-[45px] flex flex-col justify-between group">
                  ...
                  <div className="flex gap-1 justify-center items-center h-3">
                    {items.slice(0, 4).map((item, idx) => (
                      <div key={idx} className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: getHexColor(item.categoryColor) }}></div>
                    ))}
                    {items.length > 4 && (
                        ...
                        <div className="space-y-1.5 max-h-32 overflow-y-auto pr-1 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-gray-900 [&::-webkit-scrollbar-thumb]:bg-gray-700 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-gray-600">
                          {items.map((item, idx) => (
                            <div key={idx} className="flex gap-1.5 items-start text-xs">
                              <span className="w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0" style={{ backgroundColor: getHexColor(item.categoryColor) }} />
            ...
            {/* CELLE NASCOSTE (End) per chiudere la griglia senza renderle visibili */}
            {Array.from({ length: 42 - (mainFirstDayIndex + mainDaysInMonth) }).map((_, i) => <div key={`empty-end-${i}`} className="p-2 border-transparent min-h-[45px]"></div>)}
          </div>
        </div>
      ) : (
                  ...
                  >
                    ...
                    {hoveredDay === day.dateStr && popupRect && rawDayEvents.length > 0 && (() => {
                      ...
                      rawDayEvents.forEach(({ ev, seg }) => {
                        // Se l'evento è impostato su "Tutto il giorno", non allarghiamo nulla
                        if (ev.tutto_il_giorno) return;

                        // LA MAGIA: Calcoliamo l'ora di inizio e allarghiamo SOLO QUELLA!
                        const startH = Math.floor(seg.startMins / 60);
                        filledHours.add(startH);
                      });
                    ...
                    {rawDayEvents.map(({ ev, seg }, idx) => {
                      ...
                      return (
                        <div 
                          ...
                          style={{ 
                            backgroundColor: getHexColor(ev.categoryColor),
                            top: seg.top, 
                            marginTop: '0.4rem', 
