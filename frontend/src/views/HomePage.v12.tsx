...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    taskPadre.forEach((t: any) => {
      ...
      let sottotaskPromossePerData: any[] = [];
      let sottotaskUrgentiSenzaData: any[] = [];

      if (sottotaskAttive.length > 0) {
        // 1. Promozione per Data (la tua logica esistente)
        ...
        if (tempoMinimo < tempoPadre) {
          sottotaskPromossePerData = sottotaskConScadenza.filter((sub: any) => 
            new Date(sub.data_scadenza.substring(0, 10)).getTime() === tempoMinimo
          );
        }

        // 2. NUOVA REGOLA: Estraiamo TUTTE le sottotask con Priorità Alta e Senza Data
        sottotaskUrgentiSenzaData = sottotaskAttive.filter((sub: any) => 
      ...
      if (sottotaskPromossePerData.length > 0) {
        sottotaskPromossePerData.forEach((sub: any) => {
          const catSubTrovata = dbCategories.find((c: any) => c.id === sub.category_id);
          const scadenzaSubStr = sub.data_scadenza.substring(0, 10);
          taskDaMostrare.push({
            id: sub.id, title: sub.titolo, 
            deadline: scadenzaSubStr.split('-').reverse().join('/'), dateStr: scadenzaSubStr,
            done: sub.fatto, priority: sub.priorita,
            category: catSubTrovata ? catSubTrovata.name : nomeCategoria,
            categoryColor: catSubTrovata ? catSubTrovata.colore : coloreCategoria,
            description: sub.descrizione || '', location: sub.luogo || '', parent_id: sub.parent_id,
        ...
        taskDaMostrare.push({
          id: t.id, title: t.titolo,
          deadline: scadenzaPadreStr ? scadenzaPadreStr.split('-').reverse().join('/') : 'Nessuna',
          dateStr: scadenzaPadreStr || inizioPadreStr,
          done: t.fatto, priority: t.priorita,
          category: nomeCategoria, categoryColor: coloreCategoria,
          description: t.descrizione || '', location: t.luogo || '', parent_id: t.parent_id,
      ...
      if (sottotaskUrgentiSenzaData.length > 0) {
        sottotaskUrgentiSenzaData.forEach((sub: any) => {
          const catSubTrovata = dbCategories.find((c: any) => c.id === sub.category_id);
          taskDaMostrare.push({
            id: sub.id, title: sub.titolo, 
            deadline: 'Nessuna', dateStr: '',
            done: sub.fatto, priority: sub.priorita,
            category: catSubTrovata ? catSubTrovata.name : nomeCategoria,
            categoryColor: catSubTrovata ? catSubTrovata.colore : coloreCategoria,
            description: sub.descrizione || '', location: sub.luogo || '', parent_id: sub.parent_id,
            isPromotedSubtask: true,
            isUrgentFromSubtask: true // Questo farà comparire il ⚠️ (se l'avevi impostato)
          });
        });
      }
