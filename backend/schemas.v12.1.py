    ...
    def validate_password_change(self) -> "UserSettingsUpdate":
        ...
        if any(value is not None for value in provided):
            if not all(value is not None for value in provided):
                raise ValueError("Per cambiare password devi fornire current_password, new_password e confirm_new_password.")
...
class ConfigCodeCreate(BaseModel):
...
class ConfigCodeUpdate(BaseModel):
...
class ShoppingGroupCreate(BaseModel):
...
class ShoppingGroupUpdate(BaseModel):
...
class ShoppingGroupMemberCreate(BaseModel):
    user_id: int
    role_id: int


class ShoppingGroupMemberUpdate(BaseModel):
    role_id: int
...
class ShoppingPriceUpdate(BaseModel):
    ...
    shopping_list_item_id: Optional[int] = None
    product_name_original: Optional[str] = None
    product_name_normalized: Optional[str] = None
    supplier_id: Optional[int] = None
    purchase_date: Optional[date] = None
...
class ShoppingSupplierCreate(BaseModel):
    name: str = Field(..., min_length=1, max_length=255)
    status_id: Optional[int] = None
...
class ShoppingSupplierUpdate(BaseModel):
    name: Optional[str] = Field(None, min_length=1, max_length=255)
    status_id: Optional[int] = None
    updated_by_user_id: Optional[int] = None
    deleted_at: Optional[datetime] = None
