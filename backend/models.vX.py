...
class Task(Base):
    ...
    def calculate_depth(self, db_session) -> int:
        # 
        # Calcola la profondità del task risalendo la gerarchia dei parent.
        # Esegue UNA SOLA query ricorsiva (CTE) invece di fare query multiple in un ciclo while.
        # 
        ...
        # 1. Parte base della ricerca ricorsiva: partiamo dal task padre attuale
        ancestor_cte = (
        ...
        # 2. Parte ricorsiva: uniamo i padri dei padri
        recursive_part = select(Task.id, Task.parent_id).join(
            ancestor_cte, Task.id == ancestor_cte.c.parent_id
        )
        
        # 3. Uniamo il tutto con union_all
        ancestor_cte = ancestor_cte.union_all(recursive_part)

        # 4. Chiediamo al DB di contare quanti antenati ha trovato
        count_query = select(func.count()).select_from(ancestor_cte)
        total_ancestors = db_session.scalar(count_query)

        # La profondità è il numero di antenati + 1 (il task stesso)
