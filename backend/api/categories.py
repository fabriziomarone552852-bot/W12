...
def _find_accessible_category_by_name(
    name: str,
    ...
    if exclude_category_id is not None:
        query = query.filter(models.Category.id != exclude_category_id)
    return query.first()
...
def create_category(
    ...
    existing = _find_accessible_category_by_name(category_in.name, current_user, db)
    if existing:
        raise HTTPException(
            status_code=400,
...
def update_category(
    ...
    if "name" in update_data:
        existing = _find_accessible_category_by_name(
            update_data["name"],
            current_user,
            db,
