...
def get_events(
    ...
    if descrizione:
        base_query = base_query.filter(
            models.Event.descrizione.ilike(f"%{descrizione}%")
        )
    if luogo:
    ...
    if tutto_il_giorno is not None:
        base_query = base_query.filter(
            models.Event.tutto_il_giorno == tutto_il_giorno
        )
    if start_date is not None:
        base_query = base_query.filter(
            cast(models.Event.data_inizio, Date) >= start_date
        )
    if end_date is not None:
        base_query = base_query.filter(
            cast(models.Event.data_inizio, Date) <= end_date
        )
...
def delete_event(
    ...
    if not db_event:
        raise HTTPException(
            status_code=404, detail="Impegno non trovato o non accessibile"
        )
