...
def _get_habit_owned(
    ...
    # Carichiamo solo i log degli ultimi 30 giorni per evitare payload giganti
    trenta_giorni_fa = date.today() - timedelta(days=30)
    ...
    habit = (
        ...
        .options(
            ...
            with_loader_criteria(
                models.HabitLog,
                models.HabitLog.data_riferimento >= trenta_giorni_fa
            )
        )
        .filter(
...
def list_habits(
    ...
    if attive_al is not None:
        # Filtriamo direttamente in SQL usando JOIN invece di caricare tutto e scartare in Python
        query = query.join(models.HabitPeriod).filter(
            models.HabitPeriod.data_inizio <= attive_al,
            (models.HabitPeriod.data_fine == None) | (models.HabitPeriod.data_fine >= attive_al)
        )

    habits = query.order_by(models.Habit.id.desc()).all()
